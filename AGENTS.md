# AGENTS.md — BGM Forge の開発ルール

このファイルは、AIが作業を始める前に読む決まりごとです。人間向けの経緯と計測値は `HANDOVER.md` にあります。
迷ったら `HANDOVER.md` を先に読んでください。

BGM Forge は、TRPGの場面に合うループBGMをブラウザだけで自動作曲するツールです。
ネット接続・アカウント・外部APIは使いません。既定は「伴奏だけ・ループ用・合成音源」。

---

## 0. 着手前に必ず

```sh
node tests/scene-variation.cjs      # 816ケース。PASSを確認してから触る
```

PASSしない状態で作業を始めないでください。先に原因を報告すること。

---

## 1. 開発のループ

このプロジェクトで効果があったのは、**感想を数字に変えてから直す**という手順です。必ずこの順で回してください。

1. **測る** — 「〜っぽい」「うるさい」「気になる」という指摘を、楽譜の統計か音声の数値に翻訳する
2. **直す** — 最小の変更で
3. **測り直す** — 同じ指標で、改善したことを数字で示す
4. **テストに固定する** — 同じ退行が起きないアサートを `tests/scene-variation.cjs` に足す
5. **記録する** — 変更内容と計測値を `HANDOVER.md` の §3 に追記する

**「たぶん良くなった」で終わらせないこと。** 過去に、内声を絶対音域で固定したら80–250Hz帯がかえって増えた
（41.8%→51.7%）ことが計測で分かって方針を変えた、という失敗をしています。

### 測り方

楽譜の統計は Node から直接取れます。`bgm-score.js` は純粋関数だけで音を出しません。

```js
const fs=require('fs'),vm=require('vm'),path=require('path');
const score=require('./bgm-score.js');
const ctx={window:{BGM_TEST:{}},BGMScore:score};
vm.runInNewContext(fs.readFileSync('bgm-forge.js','utf8'),ctx);
const {MOODS,MODES,DEFAULTS}=ctx.window.BGM_TEST;   // UIを初期化せずAPIだけ生える
const m=MOODS.find(x=>x.id==='calm'),d=DEFAULTS.calm;
const s=score.compose({mood:m,scale:MODES[m.mode],bpm:d[0],sound:d[1],length:30,ending:'loop',lead:false},2026);
console.log(score.events(s).length);
```

音そのもの（音量の波、ループの継ぎ目、スペクトル）は実際にレンダリングしないと測れません。
ブラウザで `window.BGM_TEST={}` を定義してから `bgm-forge.js` を評価すると `BGM_TEST.render(score)` が使えます。
定数を変えた版を何通りも比べたいときは、ソースの**文字列を書き換えてから** `new Function()` で評価すると速い。
1曲のレンダリングに数十秒かかるので、結果を溜めてからまとめて取り出すこと。

画面を変えたときは、ヘッドレスChromiumでスクリーンショットを撮って目で見てください。
**820px と 390px の2幅**で見ると、折り返しの事故に気づけます。

### 聴くのは人間の仕事

音が良いかどうかはAIには判定できません。計測で示せるのは「谷が浅くなった」までです。
最後は必ず「鳴らして確かめてください」と伝えること。**聴かずに「良くなりました」と書かないこと。**

---

## 2. 触ってよいもの・いけないもの

| ファイル | 扱い |
|---|---|
| `bgm-score.js` | 作曲の中核。純粋関数のみ。**音を出す処理を入れない** |
| `bgm-forge.js` | 音声合成・再生・保存・UI配線 |
| `bgm_forge.html` | 画面とCSS。UI要素のidはここが正 |
| `bgm_forge_standalone.html` | **生成物。直接編集しない** |
| `samples/` `vendor/` | 音源とMP3エンコーダ。ライセンス表記を消さない |

`bgm_forge_standalone.html` を直接編集すると分割ソースと乖離します。実際に2026-09-16まで乖離していました。

---

## 3. 変更のたびに必ず

1. `bgm_forge.html` の `<script src="...?v=YYYYMMDD-名前-ハッシュ">` の**版を上げる**
   （上げないとブラウザが古いJSを使い続け、修正が反映されません）
   末尾の8桁は、そのファイルのsha256の先頭8桁です。手で計算する必要はありません。
   合っていなければ `node tests/scene-variation.cjs` が落ち、**貼るべき文字列をそのまま表示します**。
2. `python3 scripts/build_standalone.py` で単体版を作り直す
3. `node tests/scene-variation.cjs` を回す
   （単体版と分割ソースの一致、`?v=` の更新も、このテストが見ています。
   手順1・2を忘れると必ず落ちるので、記憶に頼る必要はありません）
4. `HANDOVER.md` の §3 に計測値つきで追記する
5. 完了報告には、実施内容・検証結果・未確認事項に加えて、次に行う作業候補を1つ以上添える
   （優先理由と、必要なら聴感確認・ユーザー判断待ちであることも明記する）

---

## 4. Git

- **mainに直接コミットする。プルリクエストを作らない。**
  短時間にPRの作成とマージを繰り返した結果、GitHubの自動検知にbot活動と判定され、アカウントが停止された実績があります。
- コミットメッセージは**日本語**。「何をしたか」を1行目に、理由と計測値を本文に。
- `git push --force`、`rebase`、`commit --amend` したあとのpushは使わない。
- 取り消しは、push前なら `git reset --soft HEAD~1`、push済みなら `git revert`。
- コミットしていない編集が消える操作（`reset --hard` など）は、実行前に日本語で確認を取る。
- 公開はリポジトリ直下の `公開する.command` をダブルクリック。未公開のコミットを表示し、ビルドとテスト後にCloudflareの中継・本体をデプロイし、`git push` します。
  本体設定は `hosting/wrangler.toml`、公開用ファイルの生成は `scripts/build_hosting.py`（原音や秘密を含めない）。
  環境によっては push を代行できないので、その場合はこれを案内すること。
- Cowork経由で触ると `.git/index.lock` などが消せずに残ります。`.git/_stale_locks/` へ退避してください。

---

## 5. 踏んだ落とし穴（同じ轍を踏まないこと）

- **度数と半音を混同しない。** `pitch(scale,d)` は音階の音数で意味が変わり、5音音階では
  4度が9半音になる（7音音階では7半音）。跳躍の制限を度数のまま書いていたため、和風だけ
  7半音以上の跳躍が16.3%まで増えていた（7音音階の場面は2.7〜4.3%）。音程の制限は半音で書くこと。
- **RMSでは持続音と減衰音の聞こえ方を比べられない。** 合唱がうるさいと言われて旋律と伴奏の
  RMS比を取ったら、儀式(+6.4dB)は幻想(+12.8dB)より低く出た。素材そのもののRMSと減衰の速さを
  見るほうが当たる。合唱だけがループする音源で、減衰しないぶん実際に前に出ていた。
- **「計測で効いている」と「聴いて良い」は別。** 儀式に息継ぎ（2小節ごとに全パート無音）を
  入れたとき、無音率は1.3%→9.4〜16.4%、実音の谷は−27.7dBまで落ちた。それでも却下された。
  数字が動いたら、必ず鳴らして確かめてもらうこと。
- **持続音はパッドだけ短くしても隙間にならない。** `hold` を 7.8→6.0 にしても無音率は
  1.2%→6.4%止まりで、最長連続はむしろ 8.0→10.7秒に悪化した。内声と低音が埋めるため。
  全パートの唯一の入口である `events()` の `add` で切るか、`FADE` で減衰させること。
- **原音（Freesoundからのダウンロード）をコミットしない。** 箏の原音は448MBあり、GitHubは
  100MB超を拒否するので push ごと失敗する。`.gitignore` 済み。
- **同梱バンクは1ページにつき1回しか復号できない。** `sampleBuffers` が復号後に `meta.pcm` を
  `delete` するため。キャッシュを作り直す経路を足すと `atob(undefined)` で落ちる。
- **複数のセッションが同時に触ることがある。** 着手前に `git status` と更新時刻を確認すること。

- **場面を足す・並べ替えるときは、添字で場面を指している所を先に探す。** 既定の場面が
  `state.mood=MOODS[1]` と添字で書かれていて、並べ替えると既定が変わるところだった（id で指すこと）。
- **「場面あたり◯組」は、設定の数ではなく音が違う数を数えること。** seedを変えながら数えると
  和音進行や色付けの違いが混ざり、実際より多く出る。seedを固定して鍵（型・和音の間隔・内声のずらし・
  和音の位置）だけ動かし、伴奏の音符列を指紋化して数えること。2026-09-22に「通常6組」と書いてあった
  場面の実数が2〜9だったことが、この測り方で分かった。組み合わせの一覧は `HANDOVER.md` §2.8b。
- **低音の3型が全部違わないと、組み合わせは頭打ちになる。** `bass:['walk','fifth','walk']` のように
  2種類しかない場面では幅が狭まることがある。ただし内声の順序・休符の位置も変わるため、
  型名だけで最大5通りと断定せず、音符列で実数を測ること。
- **`padAlt`（和音を鳴らす位置の別案）で打鍵の数を増やさない。** 荘厳を `[0,2]` にしたら
  和音が1小節3.4→6.8打になり、内声も打楽器もない場面なので曲全体の密度が倍になって
  「安定していない」と言われた。位置をずらすだけにすること。
- **幅を広げたら、場面の中の振れ幅も測ること。** 1小節あたりの音数の最大÷最小で見る。
  ほとんどの場面は1.2〜1.9倍に収まる。2倍を超えたら、同じ場面に聞こえなくなっていないか疑う。
- **場面ごとの伴奏の形は全部違っていなければならない。** `scene rhythm collision` のテストが、
  同じテンポ・音色・尺で比べて全場面の音符の並びが相異なることを見ている。場面を足すときは
  `SCENES` の `inner`/`bass`/`pad`/`hold`/`harmony` の組み合わせを既存と重ねないこと。
- **場面の数は6の倍数に保つ。** 画面が6列なので、崩すと最終行だけ余って行のまとまりが見えなくなる。
  並びは行ごとのグループ（日常／旅／情緒／緊張）、**行内は左から右が物語の順**
  （序盤→終盤）。`MOODS` の先頭に行ごとの流れをコメントで書いてある。
- **既定のテンポは必ず `TEMPOS` にある値から選ぶ。** のどかを92拍にしたとき、一覧にない値だったため
  選択欄に名前のない「92 BPM」が1つだけ紛れ込みました。
- **HTMLに書いたボタンの文言を、JSが起動時に上書きしている箇所がある。** 試聴ボタンがそうでした。
  文言を直すときは両方を探すこと。
- **減衰しない音源（笛・合唱）は、切り出しではなくループ点で伸ばす。** 箏の「撥弦の頭を切り出す」
  方式は撥弦専用。笛は `scripts/build_shinobue.py` のように、音程が安定している区間を探して
  整数周期ぶん離したゼロ交差にループ点を打つ。`sampleNote` の `loop` 対象に音色名を足すのも忘れずに。
- **場面によっては旋律が鳴らない。実録音を足すときは、その場面が実際に使うパートを確認する。**
  神楽は `DEFAULTS.kagura` に lead が無いので旋律(0)が存在しない。`RECORDED` の `parts` を
  合唱と同じ `[0]` にすると、笛が一度も鳴らないまま「入れたのに聞こえない」になる。
- **「きこえない」は音量とは限らない。まず発音経路を辿る。** 笛が聞こえないと言われたとき、
  Web Audio を差し替えたNodeのスタブで `BGM_TEST.render` を走らせ、「16発ループ再生・種類は
  shinobue・早回し1.03〜1.08倍」までは正しいことを先に確かめた。そのうえでパート別のピークを
  測ると、笛は伴奏の0.38倍・打楽器の6分の1で、25秒中10.1秒しか鳴っていなかった。
  `BGMScore.events` を差し替えてパートを1つずつ鳴らすと、パート別の音量が測れる。
- **自動抽出した音源の音高を信じない。必ず実測で検算する。** 箏は自己相関だけで基音を決めていた
  ため、5音中4音の `root` が +1.0〜+18.7半音ずれていた（倍音や隣の弦を基音と誤認）。表示を信じて
  早回しするので、和風は実際に1半音高い音や4半音低い音を鳴らしていた。自己相関は基音の整数分の1
  でも山が立つ。調和積スペクトルで、複数の窓が一致することまで見ること。テストが毎回実測している。
- **連続演奏から切り出す音源は、収録長のあいだ他の音が鳴らないかを見る。** 13分の即興から2.5秒を
  切ると、ほぼ必ず途中で別の弦が鳴る。ピーク正規化はその強いほうに合わさるので、狙った音が小さく
  なり実質「別の音の収録」になる。収録の最大振幅が立ち上がりにあることを条件にすること。
- **金属を撥く・叩く音は整数倍音で積まない。** 鐘・鉄琴・オルゴールの櫛歯は棒の振動なので、
  固有振動が整数倍にならない（棒はおよそ 1 : 2.76 : 5.40 : 8.90、弦や管は 1 : 2 : 3 : 4）。
  オルゴールを 1 : 2 : 3 で積んでいたため、澄んだ正弦の鐘にはなってもオルゴールにならなかった。
  `synthNote` は減衰の落とし先を `decay/ratio` にしているので、正しい比さえ与えれば
  「撥いた瞬間だけ高い分音が光って、あとは基音が残る」が自動で出る。
- **倍音を持たない音は、鳴らすと「ぴーん」になる。** 箏の旧#4は第2倍音が基音の2%、4kHz以上の
  エネルギーが他の2.4倍。これが旋律の56%を占めていた。音源を選ぶ基準に音質（倍音率・上倍音・
  高域比）を入れること。`samples/koto/manifest.json` に数値が残っている。
- **同梱音源の音域より上の音は、早回しで別の音色になる。** 箏は5音（最高 MIDI 70.4）しか同梱して
  いないのに和風の旋律は MIDI 88 まで上がり、+8〜+18半音で早回しされた音が細く硬い「ぴーん」に
  なっていた（2.6%、30秒1曲あたり最大10回）。再生速度が上がるぶん収録の2.5秒が1秒に縮むため。
  音源を足す音色では、`SAMPLE_LIFT` に上限を書いて `foldToBank` で折り返すか、音域を測ること。
- **同梱サンプルのファイル名のオクターブ表記は、実音と1つずれることがある。** 音高は波形から
  基本周波数を測って決めてください（`manifest.json` にMIDI音高を明示する運用）。
- **サンプルの原音は極端に小さいことがある。** フルートは原本のピークが0.05〜0.21で、
  ピーク調整をしないとほぼ聞こえませんでした。
- **iPhoneで音が出ない場合、まず本体のサイレントスイッチを疑う。** Web Audioは着信スイッチで無音になり、
  画面上はメーターも動くので壊れて見えます。アプリ側では検知できません。
- **同時に鳴らす音を音階順に並べない。** 撥弦を駆け上げる飾り（シタールのジャラーン）を
  音階順の5音で作ったら、6半音のなかに収まったまま全部鳴り続け、「変な重なりかたでうるさい」に
  なった。同時に鳴る音どうしの間隔の92%が2半音以下。弦は和音の音に張られているので、
  `d, d+2, d+4` とその上のオクターブだけを拾い、1オクターブ以上に散らすこと。
  非和音の音を混ぜると和音パートともぶつかる。
- **打楽器の生成経路はひとつではない。** 場面ごとの経路（`kicks` の表があるほう）と、伴奏が
  legacy のときだけ通る `events()` 後半の `s.drums!=='none'` の塊がある。神楽は前者しか通らない。
  打楽器に音を足すときは、その場面がどちらを通るか確かめること。「打楽器が鳴っているのだから
  この塊が動いている」は成り立たない（2026-09-20 に神楽鈴で踏んだ）。
- **音源を足したら、画面のクレジットに載せたか確かめる。** 笛を実録音に差し替えた日に、
  「使用素材・ライセンス」への追記を忘れていた。同梱音源がCC0であることが生成物をCC0と
  言い切れる根拠（§6）なので、載っていないのは表示の穴。`bgm_forge.html` の
  `bgm_forge.html` の `#license`（重ねて出す画面）と `scripts/build_standalone.py` の
  `bundled_credits()` の両方。
- **使用素材・ライセンスの画面は、中身を複製せずにそのまま出す。** 他の詳細（`data-more`）は
  `openDetail` が `innerHTML` を `#detailBody` へ複製する作りだが、単体版のこの画面は
  CC0とLGPLの全文とエンコーダー原本（base64）で **4.1MB** ある。複製すると開くたびに
  その分の文字列を作ることになる。`#license` はHTMLに置いたまま `hidden` を外すだけにしてある
  （実測: 単体版で開くまで157ms、JSエラー0）。
- **`bgm_forge.html` はエディタの自動整形で全体が書き換わることがある。** 2026-09-20 に
  1ファイルまるごと整形され（2,500行増）、差分が読めなくなった。中身の変化を見るときは
  **空白を全部落としてから比べる**（`re.sub(r'\s+','',t)` してから difflib）。
  テストの文字列一致も、整形で空白が入るだけで落ちる。落ちたら**テスト側を空白に鈍くする**こと
  （リダイレクトの1行で実際に踏んだ）。整形そのものは表示に影響しなかった（実測で位置・寸法とも同じ）。
- **使用素材のクレジットは `id` 付きの塊で置く。文言で探させない。** 2026-09-20 まで
  `build_standalone.py` が行頭の文言（「音源: VSCO 2 Community Edition…」）を正規表現で
  探していたため、**説明を書き換えるとビルドが壊れる**という制約があった。いまは
  `<div class="credit" id="c-vsco">` のように id で探す。新しい素材を足すときは
  `c-<名前>` の id を付けた塊で書き、**塊の中に `div` を入れない**（正規表現が終わりを取り違える）。
  単体版では `c-vsco` が同梱クレジット一式に置き換わり、`c-lame` と `c-groove` は落ちる。
- **`autoPattern` を持つ場面では `SCENES` の一部の設定が効かなくなる。** 効くのは `harmony` と `high` だけです。
- **伴奏の音量に手を入れるときは、発音の3経路すべてに掛ける。** `synthNote` / `sampleNote` / `chipTone`。
  片方だけだと音色を切り替えたときにバランスが変わります。
- **「意見を送る」は分割版だけの機能。単体起動版に持ち込まない。** 単体版はオフラインで配るものなので、
  外へ出る通信を1つも残さない方針です。`feedback:start`〜`feedback:end` で囲った塊を
  `scripts/build_standalone.py` が落とし、そのあと `"feedback" not in doc` をアサートしています。
  印を増やしたら、同スクリプトの想定個数（いまは6）も直すこと。忘れるとビルドが落ちて気づけます。
- **重ねて出す画面（ダイアログ）の寸法は、本体の文字寸法に合わせる。** 意見フォームを
  参考サイトの見た目に寄せたら、本文15px・見出し24px・幅640pxになり、1280×800で画面の高さの
  89%を占めて「大きい」と言われた。本体の本文は13px、「使い方」シートは14.5px。
  **フォームだけ別のサイトの部品に見える**のが大きさの正体なので、まず文字寸法から合わせること。
- **送信の配線を `bgm-forge.js` に書かない。** `bgm-forge.js` は単体版へ1バイトも違わず埋め込まれる
  決まりなので、あちらに書くと単体版に死んだコードが残ります。フォーム関連のJSは
  `bgm_forge.html` の印の中に直接置いてあります（このファイルで唯一のインラインscript）。
- **Webhook URL などの秘密をリポジトリにもチャットにも書かない。** 置き場は Cloudflare Worker の
  シークレット（`npx wrangler secret put DISCORD_WEBHOOK`）だけ。ページが持つのは Worker のURLで、
  これは公開してよいもの。`<meta name="feedback-endpoint">` が空なら機能ごと画面に出ません。

- **旋律とほかのパートの「休み」を別々に設計しない。** 1分以上で内声休止（約45秒）を入れたら、
  メロディの間奏（テーマ9〜12小節目、60 BPMで32〜48秒）と隣り合い、約28秒旋律がほぼ鳴らなかった。
  休みを足すときは、旋律・内声・和音のどれが同時に欠けるかを秒で並べて確かめること。
- **テーマ小節数を変える経路では旋律も作り直す。** `adjust()` が `themeBars` だけ更新して旋律を残したため、
  30秒→1分以上の延長で各周の5〜16小節目が無音になっていた。旋律は `themeBars*4` 拍周期で繰り返される。
- **ループの実尺は伴奏の周期の倍数にする。** 1小節単位で切り上げると76 BPMの1分が19小節になり、
  2〜4小節周期のパッド・和音が継ぎ目で崩れた。1分以上は4小節単位。
- **指標が拾った「崩れ」は曲中の並びと比べてから直す。** 疑惑の「継ぎ目でパッドが2小節続く」は、
  型そのものが曲中でも同じ並びで、崩れではなかった。

---

## 6. ライセンス（勝手に変えないこと）

| 対象 | ライセンス | 置き場 |
|---|---|---|
| ソースコード | MIT（Copyright (c) 2026 air-nakamoto） | `LICENSE` |
| **このツールで作った曲** | **CC0 1.0 相当・完全に自由** | `README.md` と画面の「テイクと保存」 |
| 同梱音源 VSCO 2 CE | CC0 1.0 | `samples/vsco2/LICENSE` |
| lamejs / LAME | LGPL 2.1（未改変・原本同梱） | `vendor/lamejs/` |
| Groove MIDI Dataset | CC BY 4.0（統計のみ参照。データは非収録） | 画面のクレジット |

- 生成物がCC0と言い切れるのは、**同梱音源がCC0だから**です。CC0でない音源を足すと前提が崩れます。
  音源を追加するときは必ずライセンスを確認し、CC0以外なら生成物の表示から見直すこと。
- LGPLは全文の同梱が必要です。`scripts/build_standalone.py` のクレジット埋め込みを消さないこと。
- Groove MIDI のクレジットは画面から消さないこと。

---

## 7. やらないと決めたこと

- DAW相当の編集、MIDI入力、歌声生成
- 外部API・学習モデルの利用（ブラウザ単体で完結させる方針）
  - 例外は「意見を送る」の送信だけ。**曲作り・再生・保存は外部通信なし**のままで、
    通信するのは利用者がフォームを押した瞬間だけです。単体起動版にはこの機能自体が入りません。
- プルリクエスト（§4）

---

## 8. 引き継ぎの一言

> `work/bgm-forge` のBGM Forgeを続けたい。`AGENTS.md` に開発ルール、`HANDOVER.md` の §0 に現状、
> §6.0 に次にやることがある。
> 作業前に `node tests/scene-variation.cjs` を回して現状を確認してほしい。
> 直すのは分割ソースで、最後に `scripts/build_standalone.py` で単体版を作り直し、`?v=` の版も上げること。
> コミットは日本語でmainに直接、PRは作らないで。
> 「〜っぽい」という指摘をしたら、まず計測してから直す進め方でお願いします。
