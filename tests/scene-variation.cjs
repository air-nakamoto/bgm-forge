...
const transport={takePlay:{dataset:{takePlay:'0'}},takeStop:{dataset:{takeStop:'0'}}};
context.document={querySelectorAll:sel=>[sel==='[data-take-play]'?transport.takePlay:transport.takeStop]};
syncTakeTransport();assert.equal(transport.takePlay.disabled,true);assert.equal(transport.takeStop.disabled,true);
state.take={};state.takes=[state.take];syncTakeTransport();assert.equal(transport.takePlay.disabled,false);assert.equal(transport.takeStop.disabled,true);
...
state.playTake={};syncTakeTransport();assert.equal(transport.takePlay.textContent,'再生');assert.equal(transport.takeStop.disabled,true);
state.playTake=state.take;state.busy=true;syncTakeTransport();assert.equal(transport.takePlay.disabled,true);assert.equal(transport.takeStop.disabled,true);
