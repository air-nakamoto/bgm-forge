...
2026-09-23。以下は候補作成時の計測記録。現在は夜空×オルゴールのBが試聴で採用され、本体へ反映済み。

## 採用・実装の更新

夜空×オルゴール・旋律ありのみ、補正前のミックスの増幅量を維持し、旋律RMSが伴奏RMSを超える分を抑える。
合唱・箏・他場面は従来の処理を維持。採用Bと実装出力の最大差は16bit PCMで1段階、ピーク0.10271835。
既存回帰テスト全PASS。対象曲はステムを2回生成するため処理負荷増がある。公開はまだ実行していない。
追加検証は `tests/balance-audio.cjs`（Playwright・ローカル採用B WAVが必要）。
