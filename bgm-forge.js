(() => {
  ...
  // 並びは画面の6列に合わせて4行ぶん。行が場面のまとまり、**左から右が物語の順**。
  // 並びは 2026-09-20 に本人が指定したもの。並べ替えるときは本人に確認すること。
  //   1行目 日常・明るい   明るい→のどか→水辺→街・酒場→カジノ→クリア
  //                        （導入 → 野を行く → 水を渡る → 街に着く → 山場 → 解決）
  //   2行目 旅・異郷・神秘 民族→和風→夜空→幻想→荘厳→神楽
  //                        （異郷に着く → その土地 → 夜を越す → 不思議に触れる → 厳かな場 → 神降ろし）
  //   3行目 情緒・内面     疑惑→思索→回想→悲哀→鎮魂→決断
  //                        （疑う → 調べる → 思い出す → 失う → 弔う → 決める）
  //   4行目 緊張・暗黒     暗い→機械→追跡→恐怖→儀式→緊迫
  //                        （不穏 → 無機質な奥へ → 逃走 → 逃げ場がない → 儀式の只中 → 決着）
  // 場面を足すときは、どの行の物語のどこに入るかを決めてから。6の倍数を崩すと行が見えなくなる。
  // 既定の場面は添字ではなく id で指すこと（下の state を参照）。並べ替えで既定が動かないように。
  const MOODS = [
    ...
    {id:'water',style:'drift',desc:'港、渡し船、川べり、水辺の集落。景色が開けて、少し間延びした時間に。ゆっくり揺れる伴奏の上で、柔らかいピアノが水の面を跳ねます。',name:'🌊 水辺',mode:'lydian',roots:[5,0,7],wave:'triangle',progs:[[0,3,0,4],[0,4,3,0],[3,0,4,0],[0,3,4,0],[4,0,3,0],[0,0,3,4]],drums:'ticks',energy:.5,density:.5},
    ...
    {id:'ethnic',style:'walk',desc:'異国の街、市場、砂漠、遠い土地への旅。見知らぬ文化に触れる場面や、素性の知れない相手との取引に。民族弦の乾いた撥弦と、増2度を含む東方の音階。持続する低音が下に一本通り、遠い太鼓がときおり鳴ります。',name:'🌍 民族',mode:'phrygianDominant',roots:[0,5,7],wave:'sine',progs:[[0,1,0,3],[0,3,1,0],[0,0,1,0],[0,1,3,0],[0,3,0,1],[0,0,3,1]],drums:'distant',energy:.5,density:.45},
    ...
    {id:'solemn',style:'hymn',desc:'神殿、宣誓、王の間での謁見、葬送。重い決断を下す前の静けさに。打楽器はなく、弦の伸ばした和音と低い持続音。',name:'⛪ 荘厳',mode:'ionian',roots:[9,4,2],wave:'sine',progs:[[0,3,0,4],[0,5,3,0],[3,0,4,0],[0,4,3,0],[0,2,3,4],[5,0,3,4]],drums:'none',energy:.45,density:.4},
    {id:'kagura',style:'stab',desc:'神社、呪い、封印、神降ろし、巫女の舞。人ならざるものと向き合う場面に。神楽笛の息の混じる音と低い持続音、遠い太鼓と神楽鈴で神秘を描きます。',name:'⛩️ 神楽',mode:'harmonic',roots:[0,2,7],wave:'sine',progs:[[0,3,0,4],[0,6,3,0],[3,0,6,0],[0,4,3,0],[0,3,4,6],[6,3,0,4]],drums:'heart',energy:.58,density:.4},
    {id:'doubt',style:'drift',desc:'辻褄の合わない証言、閉じたはずの扉、視線の気配。まだ何も起きていない調査の時間に、長く敷いておけます。行き来するアルペジオと、時計のような刻み。',name:'🔍 疑惑',mode:'dorian',roots:[2,9,7],wave:'sine',autoPattern:'wave',progs:[[0,1,0,6],[0,6,0,1],[1,0,6,0],[0,3,6,0],[6,1,0,6],[0,1,3,6]],drums:'ticks',energy:.42,density:.4},
    ...
    {id:'memory',style:'walk',desc:'古い記録、子供の頃の記憶、もう戻れない日々。悲哀が取り返しのつかない喪失なら、こちらは温かい懐かしさ。高域を落としたテープの、かすかな揺れ。',name:'📻 回想',mode:'mixolydian',roots:[5,10,0],wave:'sine',progs:[[0,5,3,6],[0,6,5,0],[5,0,6,3],[0,3,5,6],[6,5,0,3],[3,6,0,5]],drums:'none',energy:.45,density:.5},
    {id:'sorrow',style:'hush',desc:'別れ、喪失、回想、看取り。取り返しのつかないことが起きたあとの場面に。丸い電子音が、ゆっくり長く伸びます。',name:'💧 悲哀',mode:'aeolian',roots:[9,4,7],wave:'sine',progs:[[0,5,3,4],[0,2,5,4],[0,3,0,5],[5,3,0,4],[0,4,5,3],[2,5,0,4]],drums:'none',energy:.4,density:.45},
    ...
    {id:'decision',style:'hymn',desc:'重大な選択、対立、真相を前にした沈黙。シリアスな空気にしたい場面や、引き返せない決断を迫られる場面に。低い持続音と重い和音で、張りつめた時間を支えます。',name:'⚖️ 決断',mode:'aeolian',roots:[0,5,7],wave:'sine',progs:[[0,5,3,0],[0,2,5,0],[5,0,3,0],[0,3,5,0],[0,6,5,0],[3,0,5,0]],drums:'none',energy:.52,density:.35},
    ...
    {id:'horror',style:'stab',desc:'怪異の接近、正気度判定、開けてはいけない扉の前。逃げ場がないと分かる場面に。途切れる心音と、濁った持続音。',name:'🩸 恐怖',mode:'locrian',roots:[0,2,6],wave:'sine',progs:[[0,1,4,1],[0,4,1,0],[1,0,3,4],[0,3,1,4],[4,1,0,3],[1,4,0,1]],drums:'heart',energy:.5,density:.45},
  ...
  const VOICES={
    ...
    // オルゴールは、回る筒の突起が金属の櫛歯を撥く楽器。鳴っているのは棒の振動なので、
    // 倍音は整数倍にならない。2026-09-20 まで 4・8・12（いちばん大きい分音から見て 1・2・3倍）
    // という完全な整数倍で積んでいて、澄んだ正弦の鐘にはなってもオルゴールにならなかった。
    // 同じファイルの「ガラス・鐘」のほうが、よほど棒らしい比（1 : 2.76 : 5.4 : 8.9）だった。
    // 鳴る音域は変えない（いちばん大きい分音は基音の4倍＝2オクターブ上のまま）。その上に
    // 棒の第2・第3モードを積む。高い分音は synthNote の decay/ratio で速く減るので、
    // 撥いた瞬間だけ金属が光って、あとは基音が長く残る＝櫛歯の鳴り方になる。
    musicbox:{wet:[.52,.6],drums:.55,gains:[.3,.05,.075,.055],
      key :{wave:'sine',parts:[[4,0,.5],[11.04,7,.14],[21.6,-11,.04],[2,0,.06]],a:.0015,r:.9,decay:.07,filt:[24,11],floor:3200},
    ...
    folk:{wet:[.34,.44],drums:.6,gains:[.27,.065,.1,.07],
    ...
    choir:{wet:[.58,.66],drums:.45,gains:[.22,.05,.075,.05],
      key:{wave:'sawtooth',parts:[[1,-7,.3],[1,7,.3],[2,-3,.1],[3,4,.04]],a:.16,r:.7,hold:.58,fixed:[3600,2200],
        formants:[[620,8,1],[1080,10,.6],[2600,12,.18]],formantDry:.28,wow:4,wowHz:4.6},
      pad:{wave:'sawtooth',parts:[[1,-9,.28],[1,9,.28],[2,-4,.1],[3,5,.035]],a:.8,r:1.2,hold:.9,fixed:[1800,950],
        formants:[[500,7,1],[900,9,.5]],formantDry:.34},
  ...
  function synthNote(ctx,bus,freq,at,dur,gain,kind,pan,V,bright){
    ...
    // フォルマント＝母音の共鳴。倍音を足してローパスを掛けるだけでは「声」にならない。
    // 帯域通過を並列に足して F1〜F3 を立てると、同じ倍音構成でも人の声に寄る。
    // formants を持たない音色（既存の13種すべて）はこれまで通り素通り。
    if(spec.formants){
      const dry=ctx.createGain();dry.gain.value=spec.formantDry||.3;filter.connect(dry);dry.connect(bus);
      for(const [hz,q,level] of spec.formants){
        const bp=ctx.createBiquadFilter();bp.type='bandpass';bp.frequency.value=hz;bp.Q.value=q;
        const g=ctx.createGain();g.gain.value=level;
        filter.connect(bp);bp.connect(g);g.connect(bus);
      }
    }else filter.connect(bus);
  ...
  // 神楽鈴は小さな鈴の束。ひと振りで十数個が数ミリ秒ずれて鳴る。
  //
  // 2026-09-20 の三度目。「カーンって感じの音に聞こえる」と言われた。原因は、粒を
  // 正弦波にして0.18〜0.98秒も伸ばしていたこと。純音を長く伸ばすと、小さな鈴ではなく
  // 叩いた金属棒（カーン）になる。小さい鈴は質量が小さいので速く減衰し、音程も高く、
  // 純音ではなく帯域の狭い金属の当たり音になる。そこで粒の作りを入れ替えた。
  //   粒の本体 … 雑音を帯域通過（バンドパス）で狭めた短い当たり音。純音ではない
  //   余韻     … ごく低い音量の正弦波を少しだけ。これが無いと乾きすぎる
  // 減衰も 45〜200ミリ秒に詰めた（前は最長0.98秒＝カーンの正体）。
  //
  // バンドパスは中心以外を落とすので、同じ数字でも正弦波よりずっと小さく出る。
  // 白色雑音を中心 f0・尖り Q に通したときの実効値はおよそ √((f0/Q)/(標本化周波数/2))。
  // Q=10・f0=5kHz なら約0.15倍なので、正弦波の粒より一桁大きい音量を与えてある。
  // ここは耳で確かめる必要がある摘み（pingLevel）。
  const SUZU={
    pings:26,        // 粒（帯域通過した雑音の当たり音）の数
    tails:8,         // 余韻の正弦波の数
    spread:.2,       // 粒を撒く幅（秒）
    skew:1,          // 撒く偏り。1で均等、大きいほど頭寄り
    pingDecay:[.045,.155],   // 当たり音の減衰（秒）。ここを伸ばすとカーンに戻る
    tailDecay:[.1,.22],      // 余韻の減衰（秒）。伸ばしすぎるとここもカーンになる
    q:[7,8],                 // バンドパスの尖り［最小, 幅］
    hz:[3200,5200],          // 粒の帯域［最低, 幅］3.2〜8.4kHz
    pingLevel:[.10,.10],     // 当たり音の音量［最小, 幅］
    tailLevel:[.012,.016],   // 余韻の音量［最小, 幅］
    noise:4,noiseSpread:.2,  // 触れ合う擦れ
    width:.55};              // 左右の散らし
  function suzuTone(ctx,bus,noiseBuffer,at,dur,gain,random,pan=0){
    ...
    const when=()=>at+Math.pow(random(),SUZU.skew)*SUZU.spread;
    const pick=r=>r[0]+random()*r[1];
    // 触れ合う金属の擦れ。短い雑音を撒いて、途切れずに擦れ続けているように聞かせる。
    for(let i=0;i<SUZU.noise;i++)
      noiseSource(ctx,noiseBuffer,p,when(),.04+random()*.05,gain*.075,
        3600+random()*2400,(random()*2-1)*SUZU.width);
    // 粒の本体。帯域を狭めた雑音を短く鳴らすと、純音にならずに鈴の当たり音になる。
    for(let i=0;i<SUZU.pings;i++){
      const t=when(),end=Math.min(t+Math.min(dur,pick(SUZU.pingDecay)),ctx.length/SR-.001);
      if(end-t<.012)continue;
      const src=ctx.createBufferSource(),bp=ctx.createBiquadFilter(),
            g=ctx.createGain(),q=ctx.createStereoPanner();
      src.buffer=noiseBuffer;src.loop=true;src.loopEnd=noiseBuffer.duration;
      bp.type='bandpass';bp.frequency.value=pick(SUZU.hz);bp.Q.value=pick(SUZU.q);
      q.pan.value=(random()*2-1)*SUZU.width;
      const level=gain*pick(SUZU.pingLevel);
      g.gain.setValueAtTime(.00001,t);g.gain.linearRampToValueAtTime(level,t+.001);
      g.gain.exponentialRampToValueAtTime(.00001,end);
      src.connect(bp);bp.connect(g);g.connect(q);q.connect(p);
      src.start(t,(t*7.3)%Math.max(.01,noiseBuffer.duration-.4));src.stop(end);
    }
    // 余韻。鳴り終わりに少しだけ倍音列に乗らない正弦波を残す。長く伸ばさないこと。
    for(let i=0;i<SUZU.tails;i++){
      const t=when(),end=Math.min(t+Math.min(dur,pick(SUZU.tailDecay)),ctx.length/SR-.001);
      if(end-t<.02)continue;
      const o=ctx.createOscillator(),g=ctx.createGain(),q=ctx.createStereoPanner();
      o.type='sine';o.frequency.value=pick(SUZU.hz);
      q.pan.value=(random()*2-1)*SUZU.width;
      const level=gain*pick(SUZU.tailLevel);
      g.gain.setValueAtTime(.00001,t);g.gain.linearRampToValueAtTime(level,t+.002);
      g.gain.exponentialRampToValueAtTime(.00001,end);
      o.connect(g);g.connect(q);q.connect(p);o.start(t);o.stop(end);
    }
  }
  ...
  function sampleNote(ctx,bus,n,beat,bank,soft=false,trim=1,kinds=null){
    ...
    // 柔らかいピアノは音の出だしを50msかけて立ち上げる。録音の頭にある打弦の音（ハンマーの当たる音）は
    // 最初の10〜20msに集まっていて、18msで立ち上げると、その山をそのまま通してしまう。
    // 35msと50msを聴き比べてもらい、50msを採用（2026-09-21）。
    // 「水辺のピアノが乱暴に弾いたように聞こえる」という指摘（2026-09-21）はここ。
    // 短い音まで鈍らないよう、音符の長さの35%を上限にする。室内楽のピアノ（soft以外）は従来どおり。
    const attack=kind==='strings'?Math.min(.16,duration*.2):kind==='flute'?Math.min(.07,duration*.2):kind==='shinobue'?Math.min(.08,duration*.2):soft?Math.min(.05,duration*.35):.003;
    // 伸ばす音（ループする合唱）は、音符の中でもゆっくり引く。伴奏には FADE を入れたのに
    // 旋律を平らなままにすると、旋律だけがずっと同じ音量で鳴り続けて前に出すぎる。
    // 減衰する撥弦（箏・シタール・ピアノ）は勝手に小さくなるので何もしない。
    const sustain=SUSTAIN_FADE[kind]||1,hold=Math.max(at+attack,end-release);
    g.gain.setValueAtTime(.00001,at);g.gain.linearRampToValueAtTime(gain,at+attack);
    if(sustain<1){
      const floor=Math.max(gain*sustain,1e-5);
      g.gain.exponentialRampToValueAtTime(floor,Math.min(at+attack+2,hold));
  ...
  async function render(score,onProgress,isCancelled){
    ...
    check();
    const lead=await renderAudio(score,onProgress?p=>onProgress(p/2):null,isCancelled,true);
    check();
    const backing=await renderAudio(score,onProgress?p=>onProgress(50+p/2):null,isCancelled,false);
    check();
    const n=lead.length,L=lead.L,R=lead.R;
    let le=0,be=0,sum=0;
    for(let i=0;i<n;i++){
      le+=L[i]*L[i]+R[i]*R[i];be+=backing.L[i]**2+backing.R[i]**2;
      sum+=(L[i]+backing.L[i])**2+(R[i]+backing.R[i])**2;
    }
    const trim=le>1e-18?Math.min(1,Math.sqrt(be/le)):1;
    const gain=sum>1e-18?.075*(score.level||1)/Math.sqrt(sum/(2*n)):1;
    let peak=0;
    for(let i=0;i<n;i++){
      L[i]=Math.tanh((L[i]*trim+backing.L[i])*gain);
      R[i]=Math.tanh((R[i]*trim+backing.R[i])*gain);
      peak=Math.max(peak,Math.abs(L[i]),Math.abs(R[i]));
    }
    if(peak>.95){const limit=.95/peak;for(let i=0;i<n;i++){L[i]*=limit;R[i]*=limit}peak=.95}
    ...
    if(onProgress||isCancelled){
      for(let part=1;part<10;part++){
        ...
        try{ctx.suspend(when).then(()=>{
          // 中止しても必ず resume する。止めたままにすると startRendering の Promise が永久に
          // 解決せず、その曲ぶんのノード（2分の曲で数千個）がまるごと解放されない。
          // 中断はできない仕組みなので、最後まで回しきって結果を捨てるのが唯一の解放手段。
          if(isCancelled&&isCancelled()){
            if(!aborted){aborted=true;abortReject(Object.assign(Error('中止しました'),{cancelled:true}))}
  ...
  async function buildScores(scores,message,before=null){
    ...
    try{
      ...
      state.takes=retainTakes([...rendered,...(before?[before]:[]),...state.takes.filter(t=>t!==before)]);
  ...
  async function trySample(kind='current',offset=0){
    ...
    if(!take){
      previewBusy(kind);
      // 設定を続けて触ると試聴も次々に差し替わる。古い分は結果を捨てるだけでなく実際に止める。
      // 止めないと、2小節ぶんとはいえ OfflineAudioContext が何本も並走したまま最後まで回る。
      try{take=await render(score,null,()=>turn!==sampling)}
      catch(e){if(e&&e.cancelled)return;if(turn===sampling){previewBusy(null);status(e.message||'試聴の準備に失敗しました','error')}return}
  ...
  function draw(){
    ...
    $('takes').innerHTML='';
    const counter=$('takeCount');if(counter)counter.textContent=state.takes.length+' / 12';
    nowPlayingLabel();
    const pick=async x=>{
      if(state.busy)return;
