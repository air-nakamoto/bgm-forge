(() => {
  ...
  // 摘み。粒を増やして撒く幅を広げると「シャン」から「シャラシャラ」へ寄る。
  // 2026-09-20: bells 14→28、撒く幅 45→190ミリ秒、雑音を1発→5発に分けた（「もっと
  // シャラシャラした音にしたい」）。粒が増えるぶん1粒の音量は下げて、総量を揃えている。
  // skew は粒を撒く偏り。1より大きいと頭寄り、小さいと後ろ寄りになる。振りは最初の接触が
  // 一番密なので1より大きくする（.7 にしていたら逆に後ろへ溜まっていた）。
  const SUZU={bells:28,spread:.19,skew:1.3,decayMin:.18,decaySpan:.8,
              noise:5,noiseSpread:.22,hzLow:2300,hzSpan:4200,width:.55};
  function suzuTone(ctx,bus,noiseBuffer,at,dur,gain,random,pan=0){
    ...
    // 触れ合う金属の擦れ。ひと吹きの雑音だと「シャッ」で終わるので、短い雑音を撒いて
    // 途切れずに擦れ続けているように聞かせる。後ろほど弱くして、振りが収まる形にする。
    for(let i=0;i<SUZU.noise;i++){
      const t=at+Math.pow(random(),SUZU.skew)*SUZU.noiseSpread;
      noiseSource(ctx,noiseBuffer,p,t,.05+random()*.05,
        gain*.10*(1-.55*i/SUZU.noise),3200+random()*2600,(random()*2-1)*SUZU.width);
    }
    for(let i=0;i<SUZU.bells;i++){
      // 鈴はぴったり同時には鳴らない。前寄りに撒きつつ尾を引かせると束の揺れになる。
      const t=at+Math.pow(random(),SUZU.skew)*SUZU.spread;
      const hz=SUZU.hzLow+random()*SUZU.hzSpan;   // 倍音列に乗せない
      const decay=Math.min(dur,SUZU.decayMin+random()*SUZU.decaySpan);
      ...
      q.pan.value=(random()*2-1)*SUZU.width;      // 左右に散らすと揺れて聞こえる
      const level=gain*(.026+random()*.037);
