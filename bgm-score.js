...
(function(root){
  ...
  function longFormPlan(s){
    ...
    return {start:Math.max(0,start),end:restEnd,recoverEnd,returnAt:requested>=120?Math.round(90*s.bpm/60/4)*4:Infinity};
  }
  function longFormMode(s,beat){
    const plan=longFormPlan(s);
    if(!plan)return 'normal';
    return beat<plan.start?'normal':beat<plan.end?'rest':beat<plan.recoverEnd?'normal':beat<plan.returnAt?'alternate':'normal';
  }
