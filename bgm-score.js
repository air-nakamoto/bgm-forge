...
(function(root){
  ...
  function sceneAccompaniment(s,{bar,b,base,d,raw,inner,energy,chordFor,turn,add}){
    ...
    // 2026-09-20 三度目：「ぱらぱら感がない」「アルペジオが続く」。
    // 発音間隔 0→35ms→27ms が短すぎてコードの同時打に聞こえていた（実際のシタールは60〜150ms）。
    // 0→80ms→160msに広げ、ぱらぱら感を出す。また inner を毎小節鳴らしていたのが
    // 「アルペジオが続く」の正体だった（上の !=='ethnic' で止めている）。
    if(s.moodId==='ethnic'&&bar%4===0){
      ...
      const at=[0,.08,.16],vel=[42,34,28],pan=[-.35,0,.35];
