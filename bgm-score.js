...
(function(root){
  ...
  const SCENES={
    ...
    // 和音が1小節3.4→6.8打になり、和音が音の全部の場面なので密度が倍になって
  ...
  function compose(settings,seed){
    ...
    // 内声を、低音と同じ型に縛らずずらせるようにする。明るいは3型すべてを使い、
    // 低音3型×内声3型の9組にする。他の場面は従来どおり0/1の2通りを保つ。
    const innerChoices=m.id==='bright'?3:2;
    s.innerShift=Math.floor(rng(seed^0x494E4E52)()*innerChoices);
    ...
    s.sceneBVariant=hasSceneB?Math.floor(rng(seed^0x43414C42)()*2):0;
  ...
  function remix(source,kind,seed){
    ...
    if(kind==='accompaniment'){s.arrangementVariant=((s.arrangementVariant||0)+1+(seed>>>0)%2)%3;const innerChoices=s.moodId==='bright'?3:2;s.innerShift=((s.innerShift||0)+1)%innerChoices;if(SCENES[s.moodId]&&SCENES[s.moodId].padAlt)s.padShift=1-(s.padShift||0);s.arrangementSeed=seed;s.arpIndex=(s.arpIndex+1+seed%3)%4;s.arp=ARPS[s.arpIndex]}
  ...
  function adjust(source,settings){
    ...
    // 編集で尺だけ変えた場合も、元テイクのテーマ小節数を引き継がない。
    // 30秒テイク(8小節)を60秒へ延長したときは16小節へ作り直す。
    s.themeBars=themeBarsFor(s.bpm,s.requestedLength);
  ...
  function longFormPlan(s){
    ...
    // 速いテンポでは休止直後のA復帰が短く聞こえるため、96 BPM以上だけ2小節分延長する。
    // 遅いテンポはBの長さを圧迫しないよう従来の2小節復帰を保つ。
    const recoverEnd=restEnd+(s.bpm>=96?16:8);
    ...
    // 長さはテンポによらず約8秒（2〜4小節）。接続が1小節しかないテンポではBの終わりから休み始める。
    // 4秒（60 BPMで1小節）では短すぎて休みに聞こえない、とのユーザー評価（2026-09-26）。
    const melodyRestBars=Math.max(2,Math.min(4,Math.round(8*s.bpm/240)));
    const melodyRest=requested>=90?{start:Math.max(close1Start+4,formEnd-melodyRestBars*4),end:formEnd}:null;
  ...
  function sceneAccompaniment(s,{bar,b,base,d,raw,inner,energy,chordFor,turn,formCycle,add}){
    ...
    const calm=s.moodId==='calm',bright=s.moodId==='bright',hasSceneB=calm||bright;
    ...
    const w=calm
      ...
      : (v+(s.innerShift||0)+(innerMode==='alternate'||(innerMode==='closing'&&!finalClosing)?1+(hasSceneB?(s.sceneBVariant||0):0):0))%3;
